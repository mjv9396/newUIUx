...
const SignupForm = ({ onSelect }) => {
  ...
  useEffect(() => {
    ...
    if (sendEmailOtpResponse?.statusCode >= 400) {
      errorMessage(emailOtpResponse?.data ?? "Failed to send email OTP");
    }
  ...
  useEffect(() => {
    ...
    if (sendPhoneOtpResponse?.statusCode >= 400) {
      errorMessage(phoneOtpResponse?.data ?? "Failed to send phone OTP");
    }
  ...
  useEffect(() => {
    ...
    if (emailOtpResponse?.statusCode >= 400) {
      errorMessage(emailOtpResponse?.data ?? "Failed to verify email OTP");
    }
  ...
  useEffect(() => {
    ...
    if (phoneOtpResponse?.statusCode >= 400) {
      errorMessage(phoneOtpResponse?.data ?? "Failed to verify phone OTP");
    }
  ...
  useEffect(() => {
    ...
    if (registerData?.statusCode >= 400) {
      errorMessage(registerData?.error?.detailMessage ?? "Failed to create account");
    }

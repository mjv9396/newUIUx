...
import { addAcquirerProfile } from "../../../forms/payout";
import { endpoints } from "../../../services/apiEndpoints";
import usePost from "../../../hooks/usePost";
import { validatePayoutAcquirerProfileForm } from "../../../formValidations/payoutAcquirerProfileForm";
...
const AddAcquirerProfile = () => {
  ...
  const { postData, data, error, loading } = usePost(
    endpoints.payout.addAcquirerProfile
  );
  const handleSubmit = async (e) => {
    e.preventDefault();
    const validationError = validatePayoutAcquirerProfileForm(formData);
  ...
  useEffect(() => {
    if (data && !error) {
      successMessage("Acquirer Profile added successfully");
      setFormData(addAcquirerProfile);
    }
  }, [error, data]);
  if (currencyError) return <Error error="Error loading Currency" />;
  if (acquirerError) return <Error error="Error loading Acquirer" />;
  if (currencyLoading || acquirerLoading)
    return <Loading loading="Loading Currency or Acquirer List" />;
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="d-flex flex-wrap gap-3 align-items-center mb-5 mt-2">
            ...
            <div className={styles.input} id={styles.inputWidth}>
              ...
              />
              {errors.debitAccount && (
                <span className="errors">{errors.debitAccount}</span>
              )}{" "}
            </div>
          </div>
          ...
          <div className="mb-5">
            <div className="d-flex align-items-center gap-2 mb-3">
              <input
                ...
                id="isVirtualAccount"
                onChange={handleChange}
                checked={formData.isVirtualAccount}
              />
            ...
            {formData.isVirtualAccount && (
              <div className="d-flex my-5">
                <div className={styles.input} id={styles.inputWidth}>
                  <label htmlFor="virtualIfsc">Virtual IFSC Code</label>
                  <input
                    type="text"
                    name="virtualIfsc"
                    id="virtualIfsc"
          ...
          <div className="d-flex justify-content-end gap-3 mt-2">
            ...
            <button
              ...
              type="reset"
              onClick={() => setFormData(addAcquirerProfile)}
            >
              clear
            </button>
          </div>

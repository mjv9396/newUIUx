...
export const validateRegistrationForm = (formData) => {
  ...
  const firstNameFormatError = !firstNameEmptyError ? validateName(formData.firstName) : null;
  if (firstNameEmptyError) {
    errors.firstName = "First name is required";
  } else if (firstNameFormatError) {
    errors.firstName = firstNameFormatError;
  }

  // Last Name
  const lastNameEmptyError = validateEmpty(formData.lastName);
  const lastNameFormatError = !lastNameEmptyError ? validateName(formData.lastName) : null;
